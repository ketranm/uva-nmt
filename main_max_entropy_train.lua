...
function train()
    ...
    for epoch = 1, opt.maxEpoch do
        ...
        for i = 1, nbatches do
            ...
            model:clearState()
            local nll_batch, entropy_batch = model:forward({x, prev_y}, next_y)
            nll = nll + nll_batch
            entropy = entropy + entropy_batch
            ...
            if i % opt.reportEvery == 0 then
                ...
                local msg = 'epoch %.4f / %d   [ppl] %.4f  [entropy] %.4f  [speed] %.2f w/s [update] %.3f'
                local args = {msg, floatEpoch, opt.maxEpoch, exp(nll/i), entropy/i, totwords / timer:time().real, nupdates/1000}
